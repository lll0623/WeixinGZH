...
(function () {
    $(function () {
        ...
        var searchItems = [
                           ...
                           { 'field': 'lifeInfoTypeId', 'title': '生活信息类型', 'type': 'list', 'operator': 'equal', 'listItems': eval("(" + $('#searchpanel').data('types') + ")") },
        ...
        //通用添加数据
		var commonCreate = new CommonCreateModel($('#LifeInfoCreateModal'), abp.services.app.lifeInfo.createLifeInfo);
		commonCreate.init();
        ...
        $(commonCreate).bind('preSubmit', function (obj, arg) {
            ...
            var projects = $('form[name="lifeinfoCreateForm"').find('select[name="projects"]').val();
        ...
        //通用删除数据
        var commdel = new CommonDeleteData(abp.services.app.lifeInfo.deleteLifeInfo);
        ...
        //通用修改(先获取单条数据再修改)
        var commonUpdate = new CommonUpdateModel($('#LifeInfoUpdateModal'), abp.services.app.lifeInfo.getLifeInfo, abp.services.app.lifeInfo.updateLifeInfo);
        commonUpdate.init();
        ...
        $(commonUpdate).bind('setData', function (obj, data) {
            ...
            $('form[name="lifeinfoUpdateForm"').find('textarea[name="summary"]').html(data.summary);
            $('form[name="lifeinfoUpdateForm"').find('select[name="lifeInfoTypeId"]').val(data.lifeInfoTypeId);

        });

        $(commonUpdate).bind('preSubmit', function (obj, arg) {

            var projects = $('form[name="lifeinfoUpdateForm"').find('select[name="projects"]').val();
